...
def encode(graphs, id_encoding=True, degree_encoding=None):
    ...
    for g, graph in enumerate(graphs):
        if id_encoding is not None:
            if graph.identifiers.shape[0] != encoder_ids[g].shape[0] or graph.num_nodes != encoder_ids[g].shape[0]:
                print(f"Mismatch Occurs. GI: {graph.identifiers.shape[0]}, NI: {encoder_ids[g].shape[0]}, GN: {graph.num_nodes}")
...
def process_graph(graphs, device, kcore=None, cal_weight=None):
    adjs, diffs = [], []
    for graph in graphs:
        if cal_weight == 'node':
