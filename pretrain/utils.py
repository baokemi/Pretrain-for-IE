...
class GetCore:
    ...
    def get_edge_type(self, edge_index, edge_attr=None):
        num_edges = edge_index.size(1)
        edge_types = torch.zeros(num_edges, dtype=torch.long, device=self.device)
        
        for i in range(num_edges):
            start_node, end_node = edge_index[:, i]
            if start_node.item() not in self.weights_dic:
                continue

            if end_node.item() not in self.weights_dic:
                continue
            start_core = self.weights_dic[start_node.item()].mean()
            end_core = self.weights_dic[end_node.item()].mean()
            core_difference = abs(start_core - end_core)

            if core_difference <= 1:
                edge_type = 0
            elif core_difference <= 3:
                edge_type = 1
            else:
                edge_type = 2
