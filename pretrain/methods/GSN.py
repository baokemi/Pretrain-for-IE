...
from transformers import (AdamW, RobertaModel, RobertaTokenizer, get_linear_schedule_with_warmup)
...
def train_GSN(model, tokenizer, gconv, contrast_model, graph_dataloader, text_dataloader, optimizer, device, getcore, 
    ...
    for graph_data, text_data in zip(graph_dataloader, text_dataloader):
        ...
        _, g1 = gconv(graph_data.x, graph_data.edge_index, graph_data.batch)
        # _, g2 = gconv(*getcore.drop_node(graph_data))
...
def GSN(ts, args, path, logger, amr_dataset1, text_dataset, model_name):
    epo = args.epoch
    ...
    for t in range(1, ts + 1):
        ...
        contrast_model = DualBranchContrast(loss=loss_method, mode='T2G').to(device)

        gconv1 = Graph_GSN(input_dim=input_dim, hidden_dim=hid_units, num_layers=num_layers).to(device)
        
        optimizer1 = AdamW(list(bert_model.parameters()) + list(gconv1.parameters()), lr=lr)

        getcore1 = GetCore(amr_dataset_train, 'kcore', pn, factor, device)

        losses = []
        for epoch in tqdm(range(1, epo + 1)):
            loss = train_GSN(bert_model, tokenizer, gconv1, contrast_model, amr_dataloader, text_dataloader, optimizer1, device, getcore1, text_feature_transform)
            losses.append(loss)
