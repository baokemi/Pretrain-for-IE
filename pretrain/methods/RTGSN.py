...
def RTGSN(ts, args, path, logger, amr_dataset1, text_dataset, model_name):
    epo = args.epoch
    ...
    for t in range(1, ts + 1):
        ...
        gconv1 = Graph_RTGSN(id_dim=id_dim, input_dim=input_dim, hidden_dim=hid_units, num_layers=num_layers, num_relations=12).to(device)
        ...
        for epoch in tqdm(range(1, epo + 1)):
            loss = train_RTGSN(bert_model, tokenizer, gconv1, contrast_model, amr_dataloader, text_dataloader, optimizer1, device, getcore1, text_feature_transform)
            losses.append(loss)
...
def train_RTGSN(model, tokenizer, gconv, contrast_model, graph_dataloader, text_dataloader, optimizer, device, getcore, 
